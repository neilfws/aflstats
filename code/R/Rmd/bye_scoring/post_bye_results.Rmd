...
We also omit season 2011 so as to keep the focus on mid-season byes with 18 teams.
...
expected_counts <- list(
  `Post-bye games 2012-2023` = post_bye_games %>% 
    filter(Season > 2011) %>% 
    group_by(Game) %>% 
    filter(n() == 1) %>% 
    ungroup() %>% 
    mutate(Expected = paste0(Favourite, Result)) %>% 
    count(Expected),
  `All non-post-bye games 2012-2023` = non_post_bye_games_all %>% 
    filter(Season > 2011) %>% 
    count(Expected),
  `Same round non-post-bye games 2012-2023` = non_post_bye_games_round %>% 
    filter(Season > 2011) %>% 
    count(Expected),
  `Same date non-post-bye games 2012-2023` = non_post_bye_games_date %>% 
    filter(Season > 2011) %>% 
    count(Expected)) %>% 
...
expected_counts %>% 
  ...
  kbl(caption = paste0("Game results by favourite 2012-", 2023), digits = 2) %>%  

...
swans.date  <- strptime(swans$Date, format = "%d-%b-%Y %I:%M %p")
...
```{r tidy=TRUE, cache=TRUE, fig.height=9, fig.width=7, warning=FALSE}
...
ggplot(subset(cpother.melt, variable != "total"), aes(club, value, fill = variable)) + 
  ...
  geom_hline(y = median(club_payments_2013$total)) + scale_y_continuous(breaks = seq(0, 20000000, by = 4000000))
```

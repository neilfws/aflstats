...
ggplot(blues.2007) + geom_boxplot(aes(factor(Year), Percent, fill = Coach)) + theme_bw() + scale_fill_manual(values = c("cornflowerblue", "darkorange")) + labs(x = "Year", y = "Percentage", title = "Carlton game percentages by season under Ratten and Malthouse")
